...
return (
<style>{`* { box-sizing: border-box; } body { margin: 0; -webkit-text-size-adjust: 100%; text-size-adjust: 100%; color: #1a1a1a; background: #fff; overflow-x: hidden; } input, textarea, select { font-size: ${MOBILE_SAFE_INPUT_SIZE}px !important; } button, a, label { touch-action: manipulation; } @keyframes shake { 0%,100%{ transform:translateX(0) } 20%    { transform:translateX(-8px) } 40%    { transform:translateX(8px) } 60%    { transform:translateX(-5px) } 80%    { transform:translateX(5px) } }`}</style>
);
}
...
// ── Header ────────────────────────────────────────────────────────────────────
function Header({ modeLabel, showSummary, showMenu, syncLabel, syncLive, activeCount, reserved, seated, onSummary, onMenu, onArchive, onExit }) {
return (
<div style={{
...
fontFamily: FONT, fontSize: 9, letterSpacing: 2, padding: “6px 10px”,
border: “1px solid #e8e8e8”, borderRadius: 999, cursor: “pointer”, background: “#fff”, color: “#888”,
}}>ARCHIVE</button>
...
supabase.from(“service_archive”)
.select(“id, date, label, state”)
...
const t = entry.state?.tables || [];
const totalGuests = t.reduce((a, tbl) => a + (tbl.guests || 0), 0);
const covers = t.filter(tbl => tbl.active || tbl.arrivedAt).length;
return (
<div key={entry.id} style={{ borderBottom: “1px solid #f0f0f0”, paddingBottom: 16, marginBottom: 16 }}>
<div style={{ display: “flex”, alignItems: “center”, justifyContent: “space-between”, cursor: “pointer”, paddingBottom: 10 }}
onClick={() => setExpanded(expanded === entry.id ? null : entry.id)}>
<div style={{ display: “flex”, alignItems: “center”, gap: 14 }}>
<span style={{ fontFamily: FONT, fontSize: 15, fontWeight: 500, color: “#1a1a1a”, letterSpacing: 1 }}>{entry.label}</span>
<span style={{ fontFamily: FONT, fontSize: 10, color: “#888” }}>{covers} tables · {totalGuests} guests</span>
</div>
<span style={{ fontFamily: FONT, fontSize: 14, color: “#bbb”, transform: expanded === entry.id ? “rotate(180deg)” : “none”, display: “inline-block”, transition: “transform 0.15s” }}>▾</span>
</div>
{expanded === entry.id && (
<div style={{ display: “flex”, flexDirection: “column”, gap: 14 }}>
{t.map(tbl => (
<div key={tbl.id} style={{ padding: “12px 14px”, background: “#fafafa”, border: “1px solid #efefef”, borderRadius: 4 }}>
<div style={{ display: “flex”, alignItems: “center”, gap: 10, marginBottom: 10, flexWrap: “wrap” }}>
<span style={{ fontFamily: FONT, fontSize: 16, fontWeight: 300, color: “#1a1a1a” }}>{String(tbl.id).padStart(2,“0”)}</span>
{tbl.resName   && <span style={{ fontFamily: FONT, fontSize: 12, color: “#1a1a1a”, fontWeight: 500 }}>{tbl.resName}</span>}
{tbl.arrivedAt && <span style={{ fontFamily: FONT, fontSize: 10, color: “#4a9a6a”, border: “1px solid #b8e0c8”, borderRadius: 2, padding: “2px 6px” }}>arr. {tbl.arrivedAt}</span>}
{tbl.menuType  && <span style={{ fontFamily: FONT, fontSize: 10, color: “#333”, border: “1px solid #e0e0e0”, borderRadius: 2, padding: “2px 6px” }}>{tbl.menuType}</span>}
{tbl.guests    && <span style={{ fontFamily: FONT, fontSize: 10, color: “#888” }}>{tbl.guests} guests</span>}
{tbl.birthday  && <span style={{ fontSize: 13 }}>🎂</span>}
</div>
<div style={{ display: “flex”, flexDirection: “column”, gap: 5 }}>
{(tbl.seats || []).map(s => {
const drinks = [
…(s.glasses   || []).map(w  => ({ dot: “#aaa”,    color: “#333”,    bg: “#f5f5f5”, border: “#ddd”,    label: w.name + (w.vintage ? “ · “ + w.vintage : “”) })),
…(s.cocktails || []).map(c  => ({ dot: “#c9a8e0”, color: “#7a507a”, bg: “#f9f5ff”, border: “#dcc9ea”, label: c.name })),
…(s.spirits   || []).map(sp => ({ dot: “#d8b48c”, color: “#a07040”, bg: “#fffaf5”, border: “#e5d3bb”, label: sp.name + (sp.notes ? “ · “ + sp.notes : “”) })),
];
const extras = dishes.filter(d => s.extras?.[d.id]?.ordered);
const restr  = (tbl.restrictions || []).filter(r => r.pos === s.id);
const ws = waterStyle(s.water);
const hasAny = (s.water && s.water !== “—”) || s.pairing || drinks.length || extras.length || restr.length;
if (!hasAny) return null;
return (
<div key={s.id} style={{ display: “flex”, alignItems: “flex-start”, gap: 8, padding: “6px 0”, borderTop: “1px solid #f0f0f0” }}>
<div style={{ width: 22, height: 22, borderRadius: “50%”, flexShrink: 0, border: “1px solid #e0e0e0”, display: “flex”, alignItems: “center”, justifyContent: “center”, fontFamily: FONT, fontSize: 8, color: “#777” }}>P{s.id}</div>
<div style={{ display: “flex”, gap: 4, flexWrap: “wrap”, alignItems: “center” }}>
{s.water && s.water !== “—” && <span style={{ fontFamily: FONT, fontSize: 9, fontWeight: 600, padding: “2px 6px”, borderRadius: 2, background: ws.bg || “#f0f0f0”, border: “1px solid #e0e0e0”, color: “#1a1a1a” }}>{s.water}</span>}
{s.pairing && <span style={{ fontFamily: FONT, fontSize: 9, fontWeight: 600, padding: “2px 6px”, borderRadius: 2, border: “1px solid #e0e0e0”, color: pairingColor[s.pairing] || “#555”, background: pairingBg[s.pairing] || “#fafafa” }}>{s.pairing}</span>}
{drinks.map((d, di) => (
<span key={di} style={{ display: “inline-flex”, alignItems: “center”, gap: 3, fontFamily: FONT, fontSize: 10, color: d.color, background: d.bg, border: “1px solid “ + d.border, borderRadius: 2, padding: “2px 6px 2px 5px” }}>
<span style={{ width: 5, height: 5, borderRadius: “50%”, background: d.dot, flexShrink: 0 }} />{d.label}
</span>
))}
{extras.map(d => <span key={d.id} style={{ fontFamily: FONT, fontSize: 10, color: “#3a6a3a”, background: “#eef8ee”, border: “1px solid #aadaaa”, borderRadius: 2, padding: “2px 6px” }}>{d.name}</span>)}
{restr.map((r, ri) => <span key={ri} style={{ fontFamily: FONT, fontSize: 9, color: “#c07070”, background: “#fef0f0”, border: “1px solid #f0c0c0”, borderRadius: 2, padding: “2px 6px” }}>⚠ {r.note}</span>)}
</div>
</div>
);
})}
...
<div style={{
background: “#fff”, borderTop: “1px solid #e8e8e8”,
borderRadius: “12px 12px 0 0”, width: “100%”, maxWidth: 680,
maxHeight: “92vh”, overflow: “hidden”,
boxShadow: “0 -4px 40px rgba(0,0,0,0.10)”,
display: “flex”, flexDirection: “column”,
}} onClick={e => e.stopPropagation()}>
<div style={{ width: 36, height: 4, borderRadius: 2, background: “#e0e0e0”, margin: “12px auto 0” }} />
<div style={{ display: “flex”, alignItems: “center”, justifyContent: “space-between”, padding: “16px 24px 12px”, flexShrink: 0, borderBottom: “1px solid #f0f0f0” }}>
<div>
<div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 4, color: “#555”, marginBottom: 4 }}>ADMIN</div>
<div style={{ fontFamily: FONT, fontSize: 16, fontWeight: 500, color: “#1a1a1a”, letterSpacing: 1 }}>ARCHIVE</div>
</div>
<div style={{ display: “flex”, gap: 8 }}>
<button onClick={onArchiveAndClear} style={{
fontFamily: FONT, fontSize: 9, letterSpacing: 1.5, padding: “7px 12px”,
border: “1px solid #c8a96e”, borderRadius: 2, cursor: “pointer”, background: “#fffbf5”, color: “#a07040”,
...
{loading ? (
<div style={{ fontFamily: FONT, fontSize: 11, color: “#bbb”, textAlign: “center”, marginTop: 40 }}>loading…</div>
) : !supabase ? (
<div style={{ fontFamily: FONT, fontSize: 11, color: “#bbb”, textAlign: “center”, marginTop: 40 }}>archive requires Supabase connection</div>
) : entries.length === 0 ? (
<div style={{ fontFamily: FONT, fontSize: 11, color: “#bbb”, textAlign: “center”, marginTop: 40 }}>no archived days yet</div>
) : entries.map(renderDay)}
</div>
<div style={{ padding: “12px 24px 20px”, borderTop: “1px solid #f0f0f0”, flexShrink: 0 }}>
<button onClick={onClose} style={{
width: “100%”, fontFamily: FONT, fontSize: 10, letterSpacing: 2,
padding: “12px”, border: “1px solid #e8e8e8”, borderRadius: 2, cursor: “pointer”, background: “#fff”, color: “#444”,
}}>CLOSE</button>
</div>
</div>
</div>
...
const copyText = () => {
const lines = [”=== MILKA SERVICE SUMMARY ===”,
new Date().toLocaleDateString(“sl-SI”) + “ · “ + new Date().toLocaleTimeString(“sl-SI”, { hour: “2-digit”, minute: “2-digit” }), “”];
active.forEach(t => {
lines.push(“TABLE “ + String(t.id).padStart(2,“0”) +
(t.resName  ? “ · “ + t.resName  : “”) +
(t.arrivedAt ? “ · arr. “ + t.arrivedAt : “”) +
(t.menuType ? “ · “ + t.menuType + “ menu” : “”));
if (t.bottleWine) lines.push(”  Bottle: “ + t.bottleWine.name + “ · “ + t.bottleWine.producer + “ · “ + t.bottleWine.vintage);
(t.seats || []).forEach(s => {
const parts = [];
if (s.water && s.water !== “—”) parts.push(”[” + s.water + “]”);
if (s.pairing) parts.push(”[” + s.pairing + “]”);
(s.glasses   || []).forEach(w  => parts.push(“Glass: “ + w.name));
(s.cocktails || []).forEach(c  => parts.push(“Cocktail: “ + c.name));
(s.spirits   || []).forEach(sp => parts.push(“Spirit: “ + sp.name));
dishes.filter(d => s.extras?.[d.id]?.ordered).forEach(d =>
parts.push(d.name + (s.extras[d.id].pairing && s.extras[d.id].pairing !== “—” ? “ (” + s.extras[d.id].pairing + “)” : “”)));
if (parts.length) lines.push(”  P” + s.id + “ “ + parts.join(” | “));
});
lines.push(””);
});
...
<div>
<div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 4, color: “#555”, marginBottom: 4 }}>TODAY</div>
<div style={{ fontFamily: FONT, fontSize: 16, fontWeight: 500, color: “#1a1a1a”, letterSpacing: 1 }}>SERVICE SUMMARY</div>
</div>
<button onClick={copyText} style={{
fontFamily: FONT, fontSize: 9, letterSpacing: 2, padding: “7px 14px”,
border: “1px solid #e0e0e0”, borderRadius: 2, cursor: “pointer”, background: “#fff”, color: “#1a1a1a”,
}}>COPY</button>
</div>
<div style={{ overflowY: “auto”, padding: “0 24px 32px”, flex: 1 }}>
{active.length === 0 ? (
<div style={{ fontFamily: FONT, fontSize: 12, color: “#999”, textAlign: “center”, marginTop: 40 }}>no active tables</div>
) : active.map(t => (
<div key={t.id} style={{ borderBottom: “1px solid #f0f0f0”, paddingBottom: 20, marginBottom: 20 }}>
<div style={{ display: “flex”, alignItems: “center”, gap: 12, marginBottom: 14, flexWrap: “wrap” }}>
<span style={{ fontFamily: FONT, fontSize: 22, fontWeight: 300, color: “#1a1a1a”, letterSpacing: 2 }}>{String(t.id).padStart(2,“0”)}</span>
{t.resName   && <span style={{ fontFamily: FONT, fontSize: 13, color: “#1a1a1a”, fontWeight: 500 }}>{t.resName}</span>}
{t.arrivedAt && <span style={{ fontFamily: FONT, fontSize: 10, color: “#4a9a6a”, border: “1px solid #b8e0c8”, borderRadius: 2, padding: “2px 7px” }}>arr. {t.arrivedAt}</span>}
{t.menuType  && <span style={{ fontFamily: FONT, fontSize: 10, color: “#333”, border: “1px solid #e0e0e0”, borderRadius: 2, padding: “2px 7px” }}>{t.menuType} menu</span>}
{t.guestType === “hotel” && t.room && <span style={{ fontFamily: FONT, fontSize: 10, color: “#a07040”, border: “1px solid #d4b888”, borderRadius: 2, padding: “2px 7px” }}>Hotel #{t.room}</span>}
{t.birthday  && <span style={{ fontSize: 14 }}>🎂</span>}
</div>
{t.bottleWine && (
<div style={{ marginBottom: 10, padding: “8px 12px”, background: “#fafafa”, border: “1px solid #ebebeb”, borderRadius: 2 }}>
<span style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 2, color: “#888”, marginRight: 8 }}>BOTTLE</span>
<span style={{ fontFamily: FONT, fontSize: 12, color: “#1a1a1a” }}>{t.bottleWine.name} · {t.bottleWine.producer} · {t.bottleWine.vintage}</span>
</div>
)}
<div style={{ display: “flex”, flexDirection: “column”, gap: 6 }}>
{(t.seats || []).map(s => {
...
border: “1px solid “ + (restr.length ? “#f0c0c0” : “#e0e0e0”),
display: “flex”, alignItems: “center”, justifyContent: “center”,
fontFamily: FONT, fontSize: 9, color: restr.length ? “#c07070” : “#555”,
}}>P{s.id}</div>
<div style={{ display: “flex”, gap: 5, flexWrap: “wrap”, flex: 1, alignItems: “center” }}>
{s.water && s.water !== “—” && (
<span style={{ fontFamily: FONT, fontSize: 10, fontWeight: 600, padding: “3px 8px”, borderRadius: 2, background: ws.bg || “#f0f0f0”, border: “1px solid #e0e0e0”, color: “#1a1a1a” }}>{s.water}</span>
)}
{s.pairing && (
<span style={{ fontFamily: FONT, fontSize: 10, fontWeight: 600, padding: “3px 8px”, borderRadius: 2, border: “1px solid #e0e0e0”, color: pairingColor[s.pairing] || “#555”, background: pairingBg[s.pairing] || “#fafafa” }}>{s.pairing}</span>
)}
{drinks.map((d, di) => (
<span key={di} style={{ display: “inline-flex”, alignItems: “center”, gap: 4, fontFamily: FONT, fontSize: 11, color: d.color, background: d.bg, border: “1px solid “ + d.border, borderRadius: 2, padding: “3px 8px 3px 6px” }}>
...
return <span key={d.id} style={{ fontFamily: FONT, fontSize: 11, color: “#3a6a3a”, background: “#eef8ee”, border: “1px solid #aadaaa”, borderRadius: 2, padding: “3px 8px” }}>{d.name}{ex.pairing && ex.pairing !== “—” ? “ · “ + ex.pairing : “”}</span>;
})}
{restr.map((r, ri) => (
<span key={ri} style={{ fontFamily: FONT, fontSize: 10, color: “#c07070”, background: “#fef0f0”, border: “1px solid #f0c0c0”, borderRadius: 2, padding: “2px 7px” }}>⚠ {r.note}</span>
))}
</div>
</div>
);
...
// Clear the flag after React has processed the state updates (next tick)
...
setTables(p => p.map(t => t.id !== id ? t : { …t, active: false, arrivedAt: null }));
...
if (!window.confirm(“Clear ALL tables? This cannot be undone.”)) return;
const blank = initTables.map(t => ({ …t }));
setTables(blank);
setSel(null);
};
const archiveAndClearAll = async () => {
if (!window.confirm(“Archive today’s service and clear all tables?”)) return;
const dateStr = new Date().toLocaleDateString(“sl-SI”, { day: “2-digit”, month: “2-digit”, year: “numeric” });
const entry = {
date: new Date().toISOString().slice(0, 10),
label: dateStr,
tables: tables.filter(t => t.active || t.arrivedAt || t.resName || t.resTime),
};
if (supabase) {
await supabase.from(“service_archive”).insert({ date: entry.date, label: entry.label, state: entry });
}
const blank = initTables.map(t => ({ …t }));
setTables(blank);
setSel(null);
...
```
// Don't write back to Supabase if this state change came FROM Supabase
...
(async () => {
  ...
  if (data?.state) {
    // Remote data exists — apply it, suppress the echo write-back
    applyingRemoteRef.current = true;
    applyBoardState(data.state);
    setSyncStatus("live");
  }
  // If no remote row yet, do NOT seed — let the normal persist effect
  // write the current state up naturally on the next user action.
  // This prevents a blank device from overwriting real data on first open.
  else {
    setSyncStatus("live");
  }
})();
...
const hProps = { syncLabel, syncLive, activeCount: active.length, reserved, seated, onExit: switchMode, onMenu: () => setAdminOpen(true), onSummary: () => setSummaryOpen(true), onArchive: () => setArchiveOpen(true) };
...
<GlobalStyle />
<Header modeLabel=“display” showSummary={false} showMenu={false} {…hProps} />
...
<div style={{ minHeight: “100vh”, background: “#fff”, fontFamily: FONT }}>
<GlobalStyle />
<Header modeLabel={mode} showSummary={true} showMenu={mode === “admin”} {…hProps} />
...
```
  ...
  {archiveOpen && (
    <ArchiveModal
      ...
      onArchiveAndClear={() => { archiveAndClearAll(); setArchiveOpen(false); }}
