...
function SummaryModal({ tables, dishes = [], onClose }) {
  ...
  const copyText = () => {
    ...
    active.forEach(t => {
      ...
      t.seats.forEach(s => {
        ...
        const sp  = (s.spirits   || []).map(sp => sp?.name).filter(Boolean);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(4px)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "#fff", borderTop: "1px solid #e8e8e8", borderRadius: "12px 12px 0 0", padding: "24px 20px 32px", width: "100%", maxWidth: 640, maxHeight: "88vh", overflowY: "auto", boxShadow: "0 -4px 40px rgba(0,0,0,0.10)" }} onClick={e => e.stopPropagation()}>
        <div style={{ width: 36, height: 4, borderRadius: 2, background: "#e0e0e0", margin: "0 auto 20px" }} />
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          <div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 4, color: "#666" }}>SERVICE SUMMARY</div>
          <button onClick={copyText} style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 2, padding: "6px 12px", border: "1px solid #e0e0e0", borderRadius: 999, cursor: "pointer", background: "#fff", color: "#444" }}>COPY</button>
        </div>
        {active.length === 0 && <div style={{ fontFamily: FONT, fontSize: 11, color: "#999", textAlign: "center", padding: "40px 0" }}>No active tables</div>}
        {active.map(t => (
          <div key={t.id} style={{ borderBottom: "1px solid #f0f0f0", paddingBottom: 20, marginBottom: 20 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "baseline", marginBottom: 12, flexWrap: "wrap" }}>
              <span style={{ fontFamily: FONT, fontSize: 22, fontWeight: 300, color: "#1a1a1a" }}>{String(t.id).padStart(2,"0")}</span>
              {t.resName  && <span style={{ fontFamily: FONT, fontSize: 14, fontWeight: 500, color: "#1a1a1a" }}>{t.resName}</span>}
              {t.arrivedAt && <span style={{ fontFamily: FONT, fontSize: 11, color: "#4a9a6a" }}>arr. {t.arrivedAt}</span>}
              {t.menuType  && <span style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 1, padding: "2px 7px", border: "1px solid #e8e8e8", borderRadius: 2, color: "#444" }}>{t.menuType}</span>}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {t.seats.map(s => {
                ...
                const restr  = (t.restrictions || []).filter(r => r.pos === s.id);
                const extras = dishes.filter(d => s.extras?.[d.id]?.ordered);
                const gs     = (s.glasses   || []).filter(Boolean);
                const cs     = (s.cocktails || []).filter(Boolean);
                const sp     = (s.spirits   || []).filter(Boolean);
                return (
                  <div key={s.id} style={{ display: "flex", gap: 8, alignItems: "flex-start", flexWrap: "wrap", padding: "8px 10px", border: "1px solid #f5f5f5", borderRadius: 2 }}>
                    <span style={{ fontFamily: FONT, fontSize: 10, fontWeight: 600, letterSpacing: 1, color: "#555", minWidth: 24 }}>P{s.id}</span>
                    <span style={{ fontFamily: FONT, fontSize: 11, padding: "2px 8px", borderRadius: 999, background: s.water === "—" ? "#f5f5f5" : ws.bg, color: s.water === "—" ? "#888" : "#1a1a1a", border: "1px solid #e0e0e0" }}>{s.water}</span>
                    {s.pairing && <span style={{ fontFamily: FONT, fontSize: 11, padding: "2px 8px", borderRadius: 999, border: "1px solid #e0e0e0", color: pairingColor[s.pairing] || "#555", background: pairingBg[s.pairing] || "#fafafa" }}>{s.pairing}</span>}
                    {gs.map((w, i)  => <span key={i} style={{ fontFamily: FONT, fontSize: 10, padding: "2px 7px", borderRadius: 2, border: "1px solid #e0e0e0", color: "#444" }}>{w.name}</span>)}
                    {cs.map((c, i)  => <span key={i} style={{ fontFamily: FONT, fontSize: 10, padding: "2px 7px", borderRadius: 2, border: "1px solid #dcc9ea", color: "#6f4d85", background: "#fcf9ff" }}>{c.name}</span>)}
                    {sp.map((sp2, i) => <span key={i} style={{ fontFamily: FONT, fontSize: 10, padding: "2px 7px", borderRadius: 2, border: "1px solid #e5d3bb", color: "#7a5030", background: "#fffaf5" }}>{sp2.name}</span>)}
        ...
        <button onClick={onClose} style={{ width: "100%", fontFamily: FONT, fontSize: 10, letterSpacing: 2, padding: "12px", border: "1px solid #e8e8e8", borderRadius: 2, cursor: "pointer", background: "#fff", color: "#444", marginTop: 8 }}>CLOSE</button>
      </div>
    </div>
  );
}
...
function ArchiveModal({ tables, dishes, onArchiveAndClear, onClearAll, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(4px)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "#fff", borderTop: "1px solid #e8e8e8", borderRadius: "12px 12px 0 0", padding: "24px 20px 32px", width: "100%", maxWidth: 640, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 -4px 40px rgba(0,0,0,0.10)" }} onClick={e => e.stopPropagation()}>
        <div style={{ width: 36, height: 4, borderRadius: 2, background: "#e0e0e0", margin: "0 auto 20px" }} />
        <div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 4, color: "#666", marginBottom: 24 }}>ARCHIVE & END OF DAY</div>

        {/* Action buttons */}
        <div style={{ display: "flex", gap: 10, marginBottom: 28 }}>
          <button onClick={onClearAll} style={{ flex: 1, fontFamily: FONT, fontSize: 10, letterSpacing: 2, padding: "12px", border: "1px solid #e8e8e8", borderRadius: 2, cursor: "pointer", background: "#fff", color: "#555" }}>CLEAR ALL</button>
          <button onClick={onArchiveAndClear} style={{ flex: 2, fontFamily: FONT, fontSize: 10, letterSpacing: 2, padding: "12px", border: "1px solid #c8a06e", borderRadius: 2, cursor: "pointer", background: "#fdf8f0", color: "#8a6030" }}>
            ARCHIVE & CLEAR ({activeTables.length} tables)
          </button>
        </div>

        {/* Archive viewer */}
        <div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 3, color: "#999", marginBottom: 14, textTransform: "uppercase" }}>Past service</div>
        {!supabase && <div style={{ fontFamily: FONT, fontSize: 11, color: "#999", padding: "20px 0" }}>Supabase not connected — archive unavailable</div>}
        {supabase && loading && <div style={{ fontFamily: FONT, fontSize: 11, color: "#999" }}>Loading…</div>}
        {supabase && !loading && entries.length === 0 && <div style={{ fontFamily: FONT, fontSize: 11, color: "#bbb", padding: "20px 0" }}>No archived services yet</div>}
        {entries.map(entry => {
          const isExp = expanded === entry.id;
          const entryTables = entry.state?.tables || [];
          const totalGuests = entryTables.reduce((a, t) => a + (t.guests || 0), 0);
          return (
            <div key={entry.id} style={{ border: "1px solid #f0f0f0", borderRadius: 2, marginBottom: 8, overflow: "hidden" }}>
              <div onClick={() => setExpanded(isExp ? null : entry.id)} style={{ padding: "12px 16px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", background: isExp ? "#fafafa" : "#fff" }}>
                <div>
                  <div style={{ fontFamily: FONT, fontSize: 12, fontWeight: 500, color: "#1a1a1a", marginBottom: 3 }}>{entry.label}</div>
                  <div style={{ fontFamily: FONT, fontSize: 10, color: "#888" }}>{entryTables.length} tables · {totalGuests} guests</div>
                </div>
                <span style={{ fontFamily: FONT, fontSize: 16, color: "#555", transform: isExp ? "rotate(180deg)" : "none", transition: "transform 0.2s", display: "inline-block" }}>⌄</span>
              </div>
              {isExp && (
                <div style={{ padding: "0 16px 16px", borderTop: "1px solid #f0f0f0" }}>
                  {entryTables.map(t => (
                    <div key={t.id} style={{ paddingTop: 14, borderBottom: "1px solid #f8f8f8", paddingBottom: 10 }}>
                      <div style={{ display: "flex", gap: 10, alignItems: "baseline", marginBottom: 8, flexWrap: "wrap" }}>
                        <span style={{ fontFamily: FONT, fontSize: 16, fontWeight: 300, color: "#1a1a1a" }}>{String(t.id).padStart(2,"0")}</span>
                        ...
                        {t.menuType  && <span style={{ fontFamily: FONT, fontSize: 9, padding: "1px 6px", border: "1px solid #e8e8e8", borderRadius: 2, color: "#555" }}>{t.menuType}</span>}
                      ...
                      {(t.seats || []).map(s => {
                        ...
                        return (
                          <div key={s.id} style={{ display: "flex", gap: 6, flexWrap: "wrap", padding: "5px 8px", marginBottom: 4, background: "#fafafa", borderRadius: 2 }}>
                            <span style={{ fontFamily: FONT, fontSize: 10, fontWeight: 600, color: "#555", minWidth: 22 }}>P{s.id}</span>
                            <span style={{ fontFamily: FONT, fontSize: 10, padding: "1px 6px", borderRadius: 999, background: s.water === "—" ? "#f0f0f0" : ws.bg, color: "#444", border: "1px solid #e0e0e0" }}>{s.water}</span>
                            {s.pairing && <span style={{ fontFamily: FONT, fontSize: 10, padding: "1px 6px", borderRadius: 999, color: pairingColor[s.pairing] || "#555", background: pairingBg[s.pairing] || "#fafafa", border: "1px solid #e0e0e0" }}>{s.pairing}</span>}
                            {extras.map(d => <span key={d.id} style={{ fontFamily: FONT, fontSize: 10, padding: "1px 6px", borderRadius: 2, border: "1px solid #88cc88", color: "#2a6a2a", background: "#e8f5e8" }}>{d.name}</span>)}
                            {restr.map((r, i) => <span key={i} style={{ fontFamily: FONT, fontSize: 10, padding: "1px 6px", borderRadius: 2, border: "1px solid #e09090", color: "#b04040", background: "#fef0f0" }}>⚠ {r.note}</span>)}
                          </div>
                        );
                      })}
                    </div>
        ...
        <button onClick={onClose} style={{ width: "100%", fontFamily: FONT, fontSize: 10, letterSpacing: 2, padding: "12px", border: "1px solid #e8e8e8", borderRadius: 2, cursor: "pointer", background: "#fff", color: "#444", marginTop: 16 }}>CLOSE</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const archiveAndClearAll = async () => {
    ...
    if (supabase) {
      await supabase.from("service_archive").insert({ date: entry.date, label: entry.label, state: { ...boardState, tables: entry.tables } });
    }
    setTables(Array.from({ length: 10 }, (_, i) => blankTable(i + 1)));
    setSel(null);
    setArchiveOpen(false);
